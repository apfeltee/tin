...
class GenRep
  ...
  def do_gsub(file, mapping)
    ...
    if changecnt > 0 then
      $stderr.printf("stats for replacing mapping in %p (%d matches in total):\n", file, changecnt)
      counter.each do |rx, cnt|
        if cnt > 0 then
          $stderr.printf(" %d matches for %p\n", cnt, rx)
        end
      end
      make_backup(file)
...
begin
  ...
  # can't use OptionParser here, because run_* run their own OptionParser instances...
  gr = GenRep.new(opts)
  verb = ARGV.shift
  if verb then
    verb = verb.downcase
